...
def copy(source, destination, move=False, verbose=False):
    """
...
#-- PURPOSE: list a directory on GSFC GMAO extranet server
def gmao_list(HOST,timeout=None,adddirlink=None,
    parser=lxml.etree.HTMLParser(),pattern='',sort=False):
    """
    List a directory on GSFC GMAO extranet server
    ...
    timeout: int or NoneType, default None
        timeout in seconds for blocking operations
    adddirlink: str or NoneType, default None
        subdirectory to list on HOST
    parser: obj, default lxml.etree.HTMLParser()
        HTML parser for lxml
    pattern: str, default ''
        regular expression pattern for reducing list
    ...
    """
    #-- add directory link
    link = 'adddirlink.php?sub={0}'.format(adddirlink) if adddirlink else ''
    #-- try listing from https
    try:
        #-- Create and submit request.
        request = urllib2.Request(posixpath.join(*HOST,link))
        tree = lxml.etree.parse(urllib2.urlopen(request,timeout=timeout),parser)
    except:
        raise Exception('List error from {0}'.format(posixpath.join(*HOST,link)))
    else:
        #-- read and parse request for files
        colnames = tree.xpath("//div/ul/li/a/text()")
        #-- reduce using regular expression pattern
        if pattern:
            i = [i for i,f in enumerate(colnames) if re.search(pattern,f)]

...
UPDATE HISTORY:
    ...
    Updated 09/2020: prse date strings "time-units since yyyy-mm-dd hh:mm:ss"
    Written 07/2020
"""
import os
import re
...
def get_leap_seconds(truncate=True):
    ...
    """
    leaps = spire_toolkit.utilities.get_data_path(['data','leap-seconds.list'])
    ...
    #-- get leap seconds
    leap_UTC,TAI_UTC=np.loadtxt(spire_toolkit.utilities.get_data_path(leaps)).T
...
def update_leap_seconds(timeout=20, verbose=False, mode=0o775):
    ...
    LOCAL = spire_toolkit.utilities.get_data_path(['data',FILE])
    HASH = spire_toolkit.utilities.get_hash(LOCAL)

    #-- try downloading from NIST ftp servers
    HOST = ['ftp.nist.gov','pub','time',FILE]
    try:
        spire_toolkit.utilities.check_ftp_connection(HOST[0])
        spire_toolkit.utilities.from_ftp(HOST,
            timeout=timeout,
            local=LOCAL,
            hash=HASH,
            verbose=verbose,
    ...
    try:
        spire_toolkit.utilities.from_http(REMOTE,
            timeout=timeout,
            local=LOCAL,
            hash=HASH,
            verbose=verbose,
